...
- (BOOL)loginStatus:(BOOL)isJumpLogin
{
    if (![self.userControl isLoginIn]){
        if (isJumpLogin) {
            [NNHLoginViewController presentInViewController:[UIApplication sharedApplication].keyWindow.rootViewController withAniamtion:YES andCompletion:nil];
        }
        return NO;
    }else{
        return YES;
...
/** 登录成功之后调用，保存用户资料 **/
- (void)userControl_saveUserDataWithUserInfo:(NNHUserModel *)userModel
{
    [self.userControl saveUserDataWithUserInfo:userModel];
}

/** 返回当前登录用户资料，无则nil **/
- (NNHUserModel *)userControl_currentUserModel
{
